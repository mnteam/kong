...
local PROXY_STATE_VERSION = "proxy-state:version"
...
local is_proxy_state_stale
local set_proxy_state_stale
local set_proxy_state_updated
...
local function get_upstream_by_id(upstream_id)
  ...
  if kong.configuration.proxy_state_consistency == "eventual" then
...
do
  ...
  create_balancer = function(upstream, recreate, history, start)
    ...
    if kong.configuration.proxy_state_consistency == "eventual" then
      local _, err = singletons.core_cache:get("balancer:upstreams:" .. upstream.id,
                      { neg_ttl = 10 }, load_upstream_into_memory, upstream.id)
      if err then
...
local function get_all_upstreams()
  ...
  if kong.configuration.proxy_state_consistency == "eventual" then
    return singletons.core_cache:get("balancer:upstreams", opts,
                              function() return nil end)
  end
...
do
  ...
  create_balancers = function(version)
    ...
    set_proxy_state_updated()

  end

  is_proxy_state_stale = function()
    local current_version = kong.core_cache:get(PROXY_STATE_VERSION, TTL_ZERO, utils.uuid)
    if current_version ~= proxy_state_version then
      return true
    end
  ...
  set_proxy_state_stale = function()
    log(DEBUG, "invalidating proxy state")
    kong.core_cache:invalidate(PROXY_STATE_VERSION)
  end


  set_proxy_state_updated = function()
    proxy_state_version = kong.core_cache:get(PROXY_STATE_VERSION, TTL_ZERO, utils.uuid)
...
local function update_proxy_state(premature)
  if premature then
    return
  end

  local opts = {
    name = "balancer_proxy_state",
    timeout = 0,
    on_timeout = "return_true",
  }

  concurrency.with_coroutine_mutex(opts, function()
    if is_proxy_state_stale() then
  ...
  local frequency = kong.configuration.proxy_state_update_frequency or 1
  local _, err = timer_at(frequency, update_proxy_state)
  if err then
